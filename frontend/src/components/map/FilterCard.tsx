...
export default class FilterCard extends Component<Props, State> {
  ...
  render() {
    return (
        <div id='filterCard' className="card shadow mb-4">
            ...
            <div className="collapse show" id="collapseCardExample">
                <div className="card-body mr-2 ml-2">
                    ...
                    <div className="form-group row">
                        <label htmlFor="crimeType" className="col-sm-4 col-form-label pr-0">범죄유형</label>
                        <select onChange={this.handleCrimeTypeSelectionChange} className="form-control custom-select-sm col-sm-8 pr-0 pl-0" id="crimeType">
                            <option value='계'>전체</option>
                            { CRIME_TYPE.map((ct) => <option key={ct}>{ct}</option>) }
                        </select>
                    </div>
                </div>
            </div>
